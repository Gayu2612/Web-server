 ...
 * @param {Object} res
 * @param {Function} next
 * @description This Function is login Store with password and confirmpassword
 */
export let verifyToken = async (req, res, next) => {
    const errors = validationResult(req);
    if (errors.isEmpty()) {
        try {
            const result = await User.findOne({ _id: req.body._id }, { userName: 1, email: 1, password: 1, isDeleted: 1, status: 1 });
            if (result) {
                req.body.loginId = result._id
            }
            return response(req, res, activity, true, 200, result, clientError.success.fetchedSuccessfully);
        } catch (err: any) {
